...
const FIELD_TYPES = [
    { type: 'student_name', label: 'Student Name', inputType: 'text', placeholder: 'Enter student name' },
    { type: 'course_name', label: 'Course Name', inputType: 'text', placeholder: 'Enter course name' },
    { type: 'completion_date', label: 'Completion Date', inputType: 'date', placeholder: '' },
    { type: 'certificate_id', label: 'Certificate ID', inputType: 'auto', placeholder: 'Auto-generated' },
    { type: 'verification_link', label: 'Verification Link', inputType: 'auto', placeholder: 'Auto-generated' },
];
...
function getPreviewValue(
    ...
    if (fieldType === 'verification_link') {
        return previewVerifCode ? `sarvarth/verify/${previewVerifCode}` : 'sarvarth/verify/XXXXXX';
    }
...
export default function TemplateDesigner() {
    ...
    const handleImageLoad = () => {
        if (imgRef.current) {
            ...
            setImgDims(dims);
            console.log(`[Designer] Image loaded: display size = ${dims.width}x${dims.height}, natural size = ${imgRef.current.naturalWidth}x${imgRef.current.naturalHeight}`);
        }
    };

    const addField = (type: string, label: string) => {
        if (fields.some(f => f.field_type === type)) {
            showToast('error', `"${label}" field already added.`);
            return;
        }

        // Default position: stagger fields vertically
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        ...
        setFields(prev =>
            prev.map(f =>
                f.id === draggingFieldId ? { ...f, position_x: newX, position_y: newY } : f
            )
        );
    ...
    const deleteTemplate = async (e: React.MouseEvent, templateId: string, tplName: string) => {
        e.stopPropagation();

        if (!window.confirm(`Are you sure you want to delete "${tplName}"?\n\nThis will also delete all certificates generated from this template.`)) {
            return;
        }
    ...
    const saveFields = async () => {
        ...
        try {
            ...
            console.log(`[Designer] Saving fields with canvas: ${displayWidth}x${displayHeight}`);
            fields.forEach(f => {
                console.log(`  -> ${f.field_type}: (${f.position_x.toFixed(1)}, ${f.position_y.toFixed(1)}) size=${f.font_size}`);
            });
            ...
            showToast('success', `Fields saved! New IDs — Certificate: ${newCertId}, Verification: ${newVerifCode}`);
        } catch {
    ...
    return (
        <div className="page">
            <div className="page-header">
                ...
                <p>Upload a certificate image, add fields, and position them on the template</p>
            </div>
            ...
            <div className="designer-container">
                ...
                <div className="designer-sidebar">
                    {/* Upload */}
                    <div className="card">
                        <h3 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Upload Template</h3>
                        ...
                        <input ref={fileInputRef} type="file" accept=".png,.jpg,.jpeg" hidden onChange={handleUpload} />
                    </div>

                    {/* Template list */}
                    {templates.length > 0 && (
                        <div className="card">
                            <h3 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Your Templates</h3>
                    ...
                    {selectedTemplate && (
                        <div className="card">
                            <h3 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Add Fields</h3>
                            <div className="field-list">
                                {FIELD_TYPES.map((ft) => {
                    ...
                    {selectedTemplate && fields.length > 0 && (
                        <div className="card">
                            <h3 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>📝 Field Preview Data</h3>
                            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '12px' }}>
                                Type values below to preview how the certificate will look. These are only for preview — actual data is entered when generating.
                            </p>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {fields.map(field => {
                                    const fieldDef = FIELD_TYPES.find(ft => ft.type === field.field_type);
                                    if (!fieldDef) return null;

                                    if (fieldDef.inputType === 'auto') {
                                        const displayValue = field.field_type === 'certificate_id'
                                            ? (previewCertId || '🔒 Save fields to generate')
                                            : field.field_type === 'verification_link'
                                                ? (previewVerifCode ? `sarvarth/verify/${previewVerifCode}` : '🔒 Save fields to generate')
                                        ...
                                        return (
                                            <div key={field.id} className="input-group">
                                                ...
                                                <div style={{
                                                    ...
                                                    fontSize: '0.82rem',
                                                    color: previewCertId ? 'var(--success)' : 'var(--text-muted)',
                                        ...
                                        <div key={field.id} className="input-group">
                                            ...
                                            {fieldDef.inputType === 'date' ? (
                                                <input
                                                    ...
                                                    onChange={(e) => updatePreview(field.field_type, e.target.value)}
                                                />
                                            ) : (
                                                <input
                                                    type="text"
                    ...
                    {selectedField && (
                        <div className="field-controls">
                            ...
                            <button className="btn btn-danger btn-sm" onClick={() => removeField(selectedField.id)}>
                                Remove Field
                            </button>
                        </div>
                    )}
                    ...
                    {/* Save */}
                    {selectedTemplate && fields.length > 0 && (
                        <button className="btn btn-primary" onClick={saveFields} disabled={saving}>
                ...
                {/* Canvas */}
                <div className={`designer-canvas ${selectedTemplate ? 'has-image' : ''}`}>
                    {!selectedTemplate ? (
                        ...
                        >
                            ...
                            />
                            {imgDims.width > 0 && fields.map((field) => (
                                <div
                                    key={field.id}
                                ...
                                >
                                    {getPreviewValue(field.field_type, previewValues, previewCertId, previewVerifCode)}
                                </div>
                            ))}
                        </div>
                    )}
