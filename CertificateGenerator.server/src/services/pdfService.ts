...
function getFieldValue(fieldType: string, data: CertificateData['certificate']): string {
    ...
    switch (fieldType) {
        ...
        case 'verification_link':
            return `sarvarth/verify/${shortCode}`;
        default:
            return fieldType;
    }
}
...
export async function generateCertificatePdf(data: CertificateData): Promise<string> {
    ...
    console.log(`[PDF] Saved canvas (display) size: ${template.canvas_width}x${template.canvas_height}`);
    console.log(`[PDF] Fields count: ${fields.length}`);

    if (fields.length === 0) {
        console.warn('[PDF] ⚠️ WARNING: No fields found! The PDF will only have the background image.');
    }

    // Load template image
    const templateImagePath = path.join(__dirname, '..', '..', template.template_image_path.replace(/^\//, ''));
    ...
    const fontMap: Record<string, { regular: typeof helvetica; bold: typeof helveticaBold }> = {
    ...
    const canvasWidth = template.canvas_width && template.canvas_width > 0 ? template.canvas_width : pdfWidth;
    const canvasHeight = template.canvas_height && template.canvas_height > 0 ? template.canvas_height : pdfHeight;
    ...
    console.log(`[PDF] Scale factors: scaleX=${scaleX.toFixed(3)}, scaleY=${scaleY.toFixed(3)}`);
    ...
    for (const field of fields) {
        ...
        console.log(`[PDF]    display: (${field.position_x.toFixed(1)}, ${field.position_y.toFixed(1)}) fontSize=${fontSize}`);
        console.log(`[PDF]    pdf:     (${pdfX.toFixed(1)}, ${pdfY.toFixed(1)}) scaledSize=${scaledFontSize}`);

        // Safety check: skip fields that would be off-page
        if (pdfX < 0 || pdfX > pdfWidth || pdfY < -50 || pdfY > pdfHeight + 50) {
            console.error(`[PDF] ❌ SKIPPING "${text}" — off-page at pdf(${pdfX.toFixed(0)}, ${pdfY.toFixed(0)})`);
            continue;
        }
